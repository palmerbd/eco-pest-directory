...
export default async function StudiosPage({
  ...
  const { studios, total, totalPages } = await getStudiosPage(pageNum, 48);
  return (
    <main>
      <section className="py-16 px-6" style={{ background: "linear-gradient(135deg, #0c1428 0%, #1a2d5a 100%)" }}>
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-xs font-bold tracking-[0.2em] uppercase mb-3" style={{ color: "#e8c560" }}>The Directory</p>
          <h1 className="font-display text-white font-bold mb-4" style={{ fontSize: "clamp(2rem, 5vw, 3rem)" }}>Private Dance Studios</h1>
          <p className="text-white/60 text-lg max-w-2xl">
            {total > 0 ? `${total.toLocaleString()} elite studios listed — search and filter to find the perfect fit.` : "Discover elite private dance studios offering instruction in ballroom, Latin, tango, and more."}
          </p>
        </div>
      </section>
      <Suspense fallback={<StudioSearchFallback />}>
        <StudioSearch studios={studios} currentPage={pageNum} totalPages={totalPages} totalStudios={total} />
      </Suspense>
      <section className="py-16 px-6 bg-white border-t border-gray-200">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="font-display font-bold text-gray-900 text-2xl mb-3">Own a Dance Studio?</h2>
          <p className="text-gray-500 mb-6">List your studio in our directory and connect with students actively searching for private lessons.</p>
          <Link href="/contact" className="inline-block px-8 py-3 rounded-lg font-bold text-gray-900 transition-all hover:brightness-110" style={{ background: "linear-gradient(135deg, #b8922a, #e8c560)" }}>Get Listed</Link>
        </div>
      </section>
